...
def get_cropped_sprite(current_sprite_index: int):
    ...
    original = Sprite.create_from_file(png_files[current_sprite_index])
    cropped = Sprite(NumpyImage.crop_alpha_texture(original.texture))
    original.position = cropped.position = window.center
    original.x -= original.width / 2 + 20
    cropped.x += cropped.width / 2 + 20
...
sprite, cropped_sprite = get_cropped_sprite(current_sprite_index)
...
def draw():
    ...
    sprite.draw()
    cropped_sprite.draw()
    draw_sprite_rects((sprite, cropped_sprite))


def on_key_press(key: KeyEvent):
    global sprite, cropped_sprite, current_sprite_index
